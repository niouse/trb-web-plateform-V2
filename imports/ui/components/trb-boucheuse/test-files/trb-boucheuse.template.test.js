...
import defaultStyles from "./../styles/trb-boucheuse.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (trb-boucheuse.template.jsx)', function() {

        before(function() {
            TrbBoucheuse = Template()()
            wrapper = mount(
                <MuiThemeProvider>
					  <TrbBoucheuse  
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#trb-boucheuse-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#trb-boucheuse-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#trb-boucheuse-title').get(0).innerHTML).to.equal("test title");
            });
        });
