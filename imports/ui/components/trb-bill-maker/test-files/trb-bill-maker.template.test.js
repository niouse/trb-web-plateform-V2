...
import defaultStyles from "./../styles/trb-bill-maker.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (trb-bill-maker.template.jsx)', function() {

        before(function() {
            TrbBillMaker = Template()()
            wrapper = mount(
                <MuiThemeProvider>
					  <TrbBillMaker  
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#trb-bill-maker-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#trb-bill-maker-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#trb-bill-maker-title').get(0).innerHTML).to.equal("test title");
            });
        });
