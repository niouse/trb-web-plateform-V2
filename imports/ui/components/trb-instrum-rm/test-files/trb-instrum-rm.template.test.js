...
import defaultStyles from "./../styles/trb-instrum-rm.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (trb-instrum-rm.template.jsx)', function() {

        before(function() {
            TrbInstrumRm = Template()()
            wrapper = mount(
                <MuiThemeProvider>
					  <TrbInstrumRm  
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#trb-instrum-rm-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#trb-instrum-rm-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#trb-instrum-rm-title').get(0).innerHTML).to.equal("test title");
            });
        });
