...
import defaultStyles from "./../styles/signup.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (signup.template.jsx)', function() {
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#signup-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#signup-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#signup-title').get(0).innerHTML).to.equal("test title");
            });
        });
