...
import defaultStyles from "./../styles/gestion-hf.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (gestion-hf.template.jsx)', function() {
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#gestion-hf-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#gestion-hf-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#gestion-hf-title').get(0).innerHTML).to.equal("test title");
            });
        });
