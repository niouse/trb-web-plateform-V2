...
import defaultStyles from "./../styles/trb-gallery-three-d.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (trb-gallery-three-d.template.jsx)', function() {

        before(function() {
            TrbGalleryThreeD = Template()()
            wrapper = mount(
                <MuiThemeProvider>
					  <TrbGalleryThreeD  
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#trb-gallery-three-d-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#trb-gallery-three-d-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#trb-gallery-three-d-title').get(0).innerHTML).to.equal("test title");
            });
        });
