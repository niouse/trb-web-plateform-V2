...
import defaultStyles from "./../styles/trb-whiteboard.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (trb-whiteboard.template.jsx)', function() {

        before(function() {
            TrbWhiteboard = Template()()
            wrapper = mount(
                <MuiThemeProvider>
					  <TrbWhiteboard  
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#trb-whiteboard-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#trb-whiteboard-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#trb-whiteboard-title').get(0).innerHTML).to.equal("test title");
            });
        });
