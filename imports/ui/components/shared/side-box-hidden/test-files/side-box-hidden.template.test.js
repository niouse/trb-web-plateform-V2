...
import defaultStyles from "./../styles/side-box-hidden.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (side-box-hidden.template.jsx)', function() {

        before(function() {
            SideBoxHidden = Template()()
            wrapper = mount(
                <MuiThemeProvider>
					  <SideBoxHidden  
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#side-box-hidden-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#side-box-hidden-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#side-box-hidden-title').get(0).innerHTML).to.equal("test title");
            });
        });
