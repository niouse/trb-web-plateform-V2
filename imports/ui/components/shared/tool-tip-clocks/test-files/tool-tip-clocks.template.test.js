...
import defaultStyles from "./../styles/tool-tip-clocks.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (tool-tip-clocks.template.jsx)', function() {

        before(function() {
            ToolTipClocks = Template()()
            wrapper = mount(
                <MuiThemeProvider>
					  <ToolTipClocks  
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#tool-tip-clocks-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#tool-tip-clocks-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#tool-tip-clocks-title').get(0).innerHTML).to.equal("test title");
            });
        });
