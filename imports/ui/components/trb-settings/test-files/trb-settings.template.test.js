...
import defaultStyles from "./../styles/trb-settings.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (trb-settings.template.jsx)', function() {

        before(function() {
            TrbSettings = Template()()
            wrapper = mount(
                <MuiThemeProvider>
					  <TrbSettings  
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#trb-settings-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#trb-settings-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#trb-settings-title').get(0).innerHTML).to.equal("test title");
            });
        });
