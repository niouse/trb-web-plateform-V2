...
import defaultStyles from "./../styles/trb-linkedin.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (trb-linkedin.template.jsx)', function() {

        before(function() {
            TrbLinkedin = Template()()
            wrapper = mount(
                <MuiThemeProvider>
					  <TrbLinkedin  
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#trb-linkedin-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#trb-linkedin-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#trb-linkedin-title').get(0).innerHTML).to.equal("test title");
            });
        });
