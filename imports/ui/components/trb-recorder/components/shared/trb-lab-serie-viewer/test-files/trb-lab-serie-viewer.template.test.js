...
import defaultStyles from "./../styles/trb-lab-serie-viewer.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (trb-lab-serie-viewer.template.jsx)', function() {

        before(function() {
            TrbLabSerieViewer = Template()()
            wrapper = mount(
                <MuiThemeProvider>
					  <TrbLabSerieViewer  
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#trb-lab-serie-viewer-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#trb-lab-serie-viewer-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#trb-lab-serie-viewer-title').get(0).innerHTML).to.equal("test title");
            });
        });
