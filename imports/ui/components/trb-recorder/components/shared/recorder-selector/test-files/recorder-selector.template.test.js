...
import defaultStyles from "./../styles/recorder-selector.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (recorder-selector.template.jsx)', function() {

        before(function() {
            RecorderSelector = Template()()
            wrapper = mount(
                <MuiThemeProvider>
					  <RecorderSelector  
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#recorder-selector-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#recorder-selector-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#recorder-selector-title').get(0).innerHTML).to.equal("test title");
            });
        });
