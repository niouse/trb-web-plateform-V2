...
import defaultStyles from "./../styles/world-cities.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (world-cities.template.jsx)', function() {

        before(function() {
            WorldCities = Template()()
            wrapper = mount(
                <MuiThemeProvider>
					  <WorldCities  
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#world-cities-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#world-cities-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#world-cities-title').get(0).innerHTML).to.equal("test title");
            });
        });
