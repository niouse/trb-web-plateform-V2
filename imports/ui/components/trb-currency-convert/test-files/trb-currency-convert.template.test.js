...
import defaultStyles from "./../styles/trb-currency-convert.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (trb-currency-convert.template.jsx)', function() {

        before(function() {
            TrbCurrencyConvert = Template()()
            wrapper = mount(
                <MuiThemeProvider>
					  <TrbCurrencyConvert  
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#trb-currency-convert-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#trb-currency-convert-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#trb-currency-convert-title').get(0).innerHTML).to.equal("test title");
            });
        });
