...
import defaultStyles from "./../styles/trb-repair-report.styles.default.js";
...
if (Meteor.isClient) {
    ...
    describe('@Template (trb-repair-report.template.jsx)', function() {

        before(function() {
            TrbRepairReport = Template()()
            wrapper = mount(
                <MuiThemeProvider>
					  <TrbRepairReport  
        ...
        describe("text and styles are set correctly", function() {
            ...
            it('DOM elements ids match with default styles keys', function() {
                for (let nodeName in defaultStyles) {
                    expect(wrapper.find('#trb-repair-report-' + nodeName).exists()).to.be.true;
                }
            });

            it('Styles provided via props.styles are correctly applied', function() {
                expect(wrapper.find('#trb-repair-report-title').props().style.color).to.equal("testColor");
            });
            it('Texts provided via props.text are correctly applied', function() {
                expect(wrapper.find('#trb-repair-report-title').get(0).innerHTML).to.equal("test title");
            });
        });
